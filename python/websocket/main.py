...
async def watch_positions(websocket: WebSocket, accounts: str = "*", manager: ConnectionManager = Depends(get_manager("positionUpdates"))):
    await loop(websocket, manager, accounts)

@app.websocket("/trades")
async def watch_trades(websocket: WebSocket, accounts: str = "*", manager: ConnectionManager = Depends(get_manager("tradeUpdates"))):
    await loop(websocket, manager, accounts)
