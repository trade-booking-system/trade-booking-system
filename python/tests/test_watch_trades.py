...
async def test_watch_trades(async_server: AsyncSystem, trades: list[schema.Trade]):
    types: list[str] = ["create" if i % 2 == 0 else "update" for i in range(6)]
    async with async_server.web as client:
        redis = async_server.redis[0]
        redis._add_channel("tradeUpdates")
        async with client.websocket_connect("/ws/watchTrades") as websocket:
