...
from utils.redis_initializer import get_redis_client_one
from utils.get_positions import get_positions, get_position

router = APIRouter()

@router.get("/{account}")
async def get_account_positions(account: str, client: Redis = Depends(get_redis_client_one)) -> PositionResponse:
...
async def get_ticker_position(account: str, ticker: str, client: Redis = Depends(get_redis_client_one)) -> Position:
