from redis import Redis
from datetime import datetime
from schema import Position
import signal
import os

def trade_handler(msg):
    data= msg["data"]
    account, stock_ticker, amount= data.split(":")
    update_position(account, stock_ticker, int(amount))

def update_position(account, stock_ticker, amount):
    key = "positions:"+account
    stock_tickers= positions_cache.get(account)
    if stock_tickers == None:
        stock_tickers= dict()
        positions_cache[account]= stock_tickers
    old_amount= stock_tickers.get(stock_ticker)
    if old_amount == None:
        position_json= client.hget(key, stock_ticker)
        old_amount= 0
        if position_json != None:
            old_amount= Position.parse_raw(position_json).amount
    new_amount= old_amount+amount
    stock_tickers[stock_ticker]= new_amount
    position= Position(account= account, stock_ticker= stock_ticker, amount= new_amount, 
                       last_aggregation_time= datetime.now(), last_aggregation_host= "host")
    client.hset(key, stock_ticker, position.json())

def termination_handler(signum, frame):
    thread.stop()
    thread.join()
    sub.close()
    client.close()

positions_cache= dict()

client = Redis(host = os.getenv("REDIS_HOST"), port = 6379, db = 1, decode_responses = True)
sub= client.pubsub(ignore_subscribe_messages= True)

sub.subscribe(**{"updatePositions":trade_handler})
thread= sub.run_in_thread()
signal.signal(signal.SIGTERM, termination_handler)
